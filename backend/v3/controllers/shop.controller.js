...
const find = async (req, h) => {
    let { q, _sort, _order, _start, _end, id, owner } = req.query
    let count = 0
    _order = 'DESC' ? '-' : ''
    q = q ? q : ''
    let lstObj
    ...
    try {
        if (id) {
            ...
            count = (await Model.find({ shopname: regex, owner: owner})).length
            lstObj = await Model.find({ shopname: regex, owner: owner })
                .skip(_start)
                .limit(_end - _start)
                .sort(`${_order}${_sort}`)
        }
